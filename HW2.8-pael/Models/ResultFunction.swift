  ...
  func getResult(text: String) -> Result {
    ...
    case "Paei":
      return Result(
        shortInfo: "Прибыль",
        picture: "0",
        characteristic: "результат хорошегоменеджмента. Производительявляется тем руководителем,который способен егообеспечить. Это преданныйсвоему делу профессионал,разбирающийся в потребностяхклиентов и технологиях,которые могут ихудовлетворить. Поэтому онвсегда знает, что и какнужно делать для достижения"
      )
    case "P---":
      return Result(
        ...
        case "Paei":
            return Result(
                ...
                characteristic: """
                    ...
                    нужно делать для достижения цели.
                """
            )
        case "P---":
            return Result(
